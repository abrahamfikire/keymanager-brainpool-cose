...
import io.mosip.kernel.signatureeeee.exception.CertificateNotValidException;
import io.mosip.kernel.signatureeeee.exception.PublicKeyParseException;
import io.mosip.kernel.signatureeeee.exception.RequestException;
...
import io.mosip.kernel.signatureeeee.service.SignatureServicev2;
import io.mosip.kernel.signatureeeee.util.SignatureUtil;
...
public class SignatureServiceImpl implements SignatureService, SignatureServicev2 {
    ...
    /**
     * Generate a MOSIP-compliant signed QR code payload (COSE_Sign1 over CBOR-encoded data)
     * @param identityData Map of identity data as per MOSIP QR spec
     * @param applicationId Application ID for key selection
     * @param referenceId Reference ID for key selection (e.g., EC_BRAINPOOLP256R1_SIGN)
     * @return byte[] COSE_Sign1 encoded bytes (ready for QR encoding)
     */
    public byte[] generateSignedQRCodePayload(Map<String, Object> identityData, String applicationId, String referenceId) {
        try {
            // 1. CBOR encode the payload
            ObjectMapper cborMapper = new ObjectMapper(new CBORFactory());
            byte[] cborPayload = cborMapper.writeValueAsBytes(identityData);

            // 2. Get the signing key from existing logic
            String timestamp = DateUtils.getUTCCurrentDateTimeString();
            SignatureCertificate certificateResponse = keymanagerService.getSignatureCertificate(applicationId,
                    Optional.of(referenceId), timestamp);
            PrivateKey privateKey = certificateResponse.getCertificateEntry().getPrivateKey();
            ...
            // 3. Create and sign COSE_Sign1
            Sign1Message msg = new Sign1Message();
            msg.SetContent(cborPayload);
            // Use ES256 or ES256-BRAINPOOL as appropriate
            msg.addAttribute(HeaderKeys.Algorithm, AlgorithmID.ECDSA_256.AsCBOR(), Attribute.PROTECTED); // Adjust if using Brainpool
            // If you want to add a key ID or x5c, do it here
            // msg.addAttribute(HeaderKeys.KID, ... , Attribute.UNPROTECTED);

            // Sign with the correct provider (HSM)
            OneKey oneKey = new OneKey(null, privateKey); // or new OneKey(publicKey, privateKey)
            msg.sign(oneKey);

            // 4. Return COSE_Sign1 bytes
            return msg.EncodeToBytes();
        } catch (Exception e) {
            LOGGER.error(SignatureConstant.SESSIONID, "QR_CODE_SIGN", SignatureConstant.BLANK,
                    "Error generating signed QR code payload", e);
            throw new RuntimeException("Failed to generate signed QR code payload", e);
        }
    }

    /**
     * Verifies a COSE_Sign1 signature using a PEM-encoded public key
     * @param coseSign1 COSE_Sign1 bytes
     * @param publicKeyPem PEM-encoded public key
     * @return true if signature is valid, false otherwise
     */
    public boolean verifyCoseSignatureWithPem(byte[] coseSign1, String publicKeyPem) {
        PublicKey publicKey = keymanagerUtil.convertToCertificate(publicKeyPem).getPublicKey();
        return keymanagerUtil.verifyCborSignature(coseSign1, publicKey);
    }
